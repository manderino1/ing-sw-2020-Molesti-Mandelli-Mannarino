...
public class MatchController extends Controller {
    ...
    private final int CAMERA_DISTANCE = 35;
    private final int CAMERA_X_ANGLE = 45;
    private final int DRAG_THRESHOLD = 10;
    private Translate pivot = new Translate(0,0, CAMERA_DISTANCE);
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        camera.translateZProperty().set(-CAMERA_DISTANCE);
        ...
        // Setup generic timeline
        planesTimeline.setCycleCount(Timeline.INDEFINITE);
        planesTimeline.setAutoReverse(true);

        // Detect difference between click and drag with a filter
        matchScene.setOnMousePressed(e -> dragStart = e.getSceneX());
        matchScene.addEventFilter(MouseEvent.MOUSE_RELEASED, e -> {
            if(abs(dragStart - e.getSceneX()) > DRAG_THRESHOLD) {
                e.consume();
            }
        });

        matchScene.setOnMouseReleased(e -> {
            matchScene.requestFocus();
            e.consume();
        });

        // Drag management for camera rotation
        matchScene.setOnMouseDragged(event -> {
            if(abs(dragStart - event.getSceneX()) > DRAG_THRESHOLD) {
                if(previousX - event.getSceneX() < 0) { // Clock/anti-clock wise detection
                    camera.getTransforms().addAll (
                            pivot,
                            new Rotate(CAMERA_X_ANGLE, Rotate.X_AXIS),
        ...
        matchScene.setOnMouseReleased(e -> {
            ...
            for(Direction direction : buildList.getBuildlist()) {
                ...
                if(newIndexes[0] == newX && newIndexes[1] == newY) { // Found the valid direction
                    ...
                    clearColor();
                    matchScene.setOnMouseReleased(e2 -> {
                        matchScene.requestFocus();
                        e2.consume();
                    });
                }
            ...
            matchScene.setOnMouseReleased(e2 -> {
                ...
                for(Direction direction : moves.get()) {
                    ...
                    if(newIndexes[0] == newX && newIndexes[1] == newY) { // Found the valid direction
                        ...
                        clearColor();
                        matchScene.setOnMouseReleased(e3 -> {
                            matchScene.requestFocus();
                            e3.consume();
                        });
                    }
    ...
    public void atlasBuild(AtlasBuildList buildList) {
        ...
        matchScene.setOnMouseReleased(e1 -> {
            matchScene.requestFocus();
        ...
        button2.setOnMouseReleased(e -> {
            ...
            matchScene.setOnMouseReleased(e3 -> {
                ...
                if(indexes[0] == myWorker1.getX() && indexes[1] == myWorker1.getY()) {
                    ...
                    clearColor();
                    matchScene.setOnMouseReleased(e4 -> {
                        matchScene.requestFocus();
                        e4.consume();
                    });
                } else if(indexes[0] == myWorker2.getX() && indexes[1] == myWorker2.getY()) {
